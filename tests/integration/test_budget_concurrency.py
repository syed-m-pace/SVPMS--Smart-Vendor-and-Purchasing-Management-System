...
async def test_concurrent_budget_consumption(client):
    ...
    # Use a unique dept name per run to avoid violating the unique(tenant_id, name) constraint
    run_id = str(uuid.uuid4())[:8]
    dept_name = f"Concur Dept {run_id}"
    dept_resp = await client.post("/api/v1/departments", json={"name": dept_name}, headers=auth)
    assert dept_resp.status_code == 201, f"Failed to create dept: {dept_resp.text}"
    ...
    try:
        ...
        # Cleanup: delete the test department to avoid accumulating test data in the DB
        await client.delete(f"/api/v1/departments/{dept_id}", headers=auth)
